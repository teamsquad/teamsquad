...
set :application, 'teamsquad' # The one you are deploying.
...
set :user, "slatter"
set :txd_primary_domain, 'teamsquad.com' # The domain you ssh to for your server.
set :lighty_port, 2541 # The port given to you by TextDrive for running lighttpd
...
server.document-root        = "/home/<%= user %>/domains/<%= application %>/current/public/" 
...
#  accesslog.filename = "/home/<%= user %>/logs/access_log" 
  ...
  fastcgi.server = ( ".fcgi" =>
    ( "localhost" =>
        (
          "socket"   => "/home/<%= user %>/domains/<%= application %>/lighttpd-fcgi.socket",
          "bin-path" => "/home/<%= user %>/domains/<%= application %>/current/public/dispatch.fcgi",
